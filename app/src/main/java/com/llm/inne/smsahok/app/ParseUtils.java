    ...
    public static void sub(Context context){
        ...
        ParsePush.subscribeInBackground(AppConfig.PARSE_CHANNEL, new SaveCallback() {
            ...
            public void done(ParseException e) {
                Log.e(TAG, "Successfully subscribed to Parse! (ON)");
            }
        });
    }
    public static void user(String username, String phone){
        ParseUser user = ParseUser.getCurrentUser();
        user.setUsername(username);

    }

    public static void unSub(Context context){
        ParseInstallation.getCurrentInstallation().saveInBackground();
        ParsePush.unsubscribeInBackground(AppConfig.PARSE_CHANNEL, new SaveCallback() {
            @Override
            public void done(ParseException e) {
                Log.e(TAG, "Successfully unsubscribed to Parse! (OFF)");
            }
        });

    }

    public static void subscribePhoneNumber(String number) {
        ParseInstallation installation = ParseInstallation.getCurrentInstallation();
        installation.put("phoneNumber", number);
